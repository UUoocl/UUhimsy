...
!(function (e, t) {
  ...
  class C {
    ...
    next() {
       ...
       // 3 ways to scroll slides into view. 1. Instant transition, 2.  
...
//default next
        ...
        //smooth scroll, speed determined by the browser
        //   this.viewportElement.scroll({
            //     top: this.viewportElement.scrollTop + this.scrollTriggerHeight,
            //     left: 0,
            //     //behavior: "instant",
            //     //behavior: "auto",
            //     behavior: "smooth",
            ...
            //custom transition behavioor and duration
            this.scrollTransitionStop = false;
            

            // animating x (margin-left) from 20 to 300, for example
            this.scrollStartY = this.viewportElement.scrollTop;
            console.log(this.scrollStartY)
            this.scrollDestY = this.viewportElement.scrollTop + this.scrollTriggerHeight;
            console.log(this.scrollDestY)
            this.scrollDuration = 100;
            console.log(typeof this.scrollDuration)
            this.scrollTransitionStart = Date.now();
            console.log(typeof this.scrollTransitionStart)
            this.scrollTransitionEnd = (this.scrollTransitionStart + this.scrollDuration);
            console.log(this.scrollTransitionEnd)
    
            requestAnimationFrame(()=> this.animateScrollTransition());    
        }

        //https://spicyyoghurt.com/tools/easing-functions
    easeInQuad (transitionPoint) {
        return  Math.round(this.scrollTriggerHeight * transitionPoint + this.scrollStartY);
    }

    animateScrollTransition() {
    //console.log("easing started")
        if (this.scrollTransitionStop){
            this.syncScrollPosition()
            return;
        } 
        
        if(this.viewportElement.scrollTop >= this.destY) {
            this.scrollTransitionStop = true;
            this.viewportElement.scrollTop = this.destY;
            console.log("transition Complete:",  this.viewportElement.scrollTop)
            return
        }
        
        let scrollNow = Date.now();
        if (scrollNow >= this.scrollTransitionEnd) this.scrollTransitionStop = true;
        let transitionPoint = (scrollNow - this.scrollTransitionStart) / this.scrollDuration;
       
        transitionPoint = transitionPoint >= 1 ? 1 : transitionPoint;
               
        let addValue = this.easeInQuad(transitionPoint);
        
        console.log("addValue", addValue)
        
        this.viewportElement.scrollTop = addValue;
        console.log(this.viewportElement.scrollTop)
